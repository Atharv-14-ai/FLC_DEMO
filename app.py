...
app.config["SECRET_KEY"] = os.environ.get("SECRET_KEY", "demo-secret-key-for-client-2024")

# Database configuration with Render fix
...
if database_url:
    ...
    # Local development fallback - SQLite for demo
...
def init_demo_data():
    ...
    # Create demo supplier if doesn't exist
    if not User.query.filter_by(role="Supplier").first():
        hashed_pwd = bcrypt.generate_password_hash("demo123").decode('utf-8')
        demo_supplier = User(username="Supplier", password=hashed_pwd, role="Supplier")
        db.session.add(demo_supplier)
        
        # Create demo intermediate
        hashed_pwd2 = bcrypt.generate_password_hash("demo123").decode('utf-8')
        demo_intermediate = User(username="demo_intermediate", password=hashed_pwd2, role="Intermediate")
        db.session.add(demo_intermediate)
        
        # Create demo components
        demo_components = [
            {"name": "Engine Block", "description": "V8 Engine Block", "flc_stock": 50},
            {"name": "Transmission", "description": "Automatic Transmission", "flc_stock": 30},
            {"name": "Brake System", "description": "Hydraulic Brake System", "flc_stock": 40},
        ]
        
        for comp_data in demo_components:
            component = Component(
                name=comp_data["name"],
                description=comp_data["description"],
                flc_stock=comp_data["flc_stock"],
                created_by=demo_supplier.id
            )
            db.session.add(component)
        
        # Create demo end users
        demo_end_users = [
            {"name": "AutoWorks Garage", "location": "Mumbai"},
            {"name": "Precision Motors", "location": "Delhi"},
            {"name": "City Auto Care", "location": "Bangalore"},
        ]
        
        for end_user_data in demo_end_users:
            end_user = EndUser(
                name=end_user_data["name"],
                location=end_user_data["location"]
            )
            db.session.add(end_user)
        
        db.session.commit()
        app.logger.info("✅ Demo data initialized successfully!")
...
# In supplier_analytics route, update the chart_data preparation:
    chart_data = {
        "dates": dates,
